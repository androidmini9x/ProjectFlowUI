...
function ProjectEdit() {
  ...
  const setValue = useCallback((name, description) => {
    formik.setFieldValue('project_name', name);
  ...
  useEffect(() => {
    ...
    const getProject = async () => {
      ...
      const data = await req.json();
      if (req.status === 200) {
        setValue(data.name, data.description);
        // formik.setFieldValue('project_name', data.name);
        // formik.setFieldValue('description', data.description);
      }
    };
    getProject();
  }, [id, authed.token, setValue]);
